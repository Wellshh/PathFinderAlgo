...
/**
 * Visual tests for D* Lite.  Each {@code @Test} prints a full
 * console-rendered walk-through with per-phase timing.
 *
 * <p>Run a single test from the IDE to inspect one scenario
...
class DStarLiteVisualTest extends AbstractVisualPathFinderTest<Point2D> {
	...
	void visualDynamicReplan() {
		...
		DStarLitePathFinder<Point2D> pf =
				(DStarLitePathFinder<Point2D>) pathFinder;
		SimpleGridSensor sensor =
				new SimpleGridSensor(groundTruth, knownMap, 3);
		...
		while (!robotPos.equals(goal)) {
			...
			if (!updates.isEmpty()) {
				System.out.printf("[Step %d] at %s -- %d edge changes%n",
						step, robotPos, updates.size());
				...
				visualizer.renderWithRobot(env, start, goal,
						currentPath, robotPos,
						"Replan at step " + step);
			}
		...
		System.out.println(robotPos.equals(goal)
