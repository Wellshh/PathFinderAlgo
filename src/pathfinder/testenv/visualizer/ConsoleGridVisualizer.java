...
/**
 * Console-based grid visualizer.
 * Renders a 2D grid as text with symbols:
...
public class ConsoleGridVisualizer implements ITestVisualizer<Point2D> {
	...
	@Override
	public void renderEnvironment(Environment<Point2D> env, Point2D start,
			Point2D goal, List<Point2D> path, String title) {
		renderWithRobot(env, start, goal, path, null, title);
	}

	@Override
	public void renderWithRobot(Environment<Point2D> env, Point2D start,
			Point2D goal, List<Point2D> path,
			Point2D robotPos, String title) {
