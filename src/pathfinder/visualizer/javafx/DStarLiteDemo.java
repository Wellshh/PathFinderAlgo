 ...
 * {@link JavaFXGridVisualizer} through the observer event system.
 *
 * <p>Usage: click or drag on the canvas to toggle obstacles.
 * Each toggle triggers an incremental replan on a background thread;
 * the updated path appears on the canvas without blocking the UI.
 */
public class DStarLiteDemo extends Application {
    ...
    private void handleObstacleToggle(int x, int y, boolean isNowObstacle) {
        ...
        if (!isNowObstacle) {
            for (Point2D neighbor : env.getNeighbors(target)) {
                updates.add(new EdgeUpdate<>(target, neighbor,
                        env.getTraversalCost(target, neighbor)));
            }
        }
