...
/**
 * Observable grid model that serves as the single source of truth for
 * Canvas rendering. Tracks per-cell dirty flags so the renderer can
 * perform incremental redraws instead of full-screen clears.
 *
 * <p>Thread-safety: mutation methods are {@code synchronized} because
 * the algorithm worker thread writes state while the JavaFX
 * Application Thread reads it via {@link #consumeDirtySet()}.
 */
public class GridViewModel {
    ...
    /**
     * Returns the state without synchronization — safe when called from
     * the FX thread after {@link #consumeDirtySet()} has been obtained.
     */
    ...
    /**
     * Atomically returns all dirty indices and clears the internal set.
     * The caller iterates the returned BitSet to repaint only changed cells.
     */
    ...
    /**
     * Clears all PATH cells back to FREE, then marks the new path.
     * Only changed cells become dirty — avoids redundant repaints.
     */
